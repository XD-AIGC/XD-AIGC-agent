...
| 4 | 出站 HTTP 白名单 | 只允许 `${TOOLBOX_BASE_URL}/*` + `${MIVO_ENDPOINT}/*` + `open.feishu.cn/*` + `llm-proxy.tapsvc.com/*`；Mivo 仅限受控 `action=mcp` 工具 |
...
- [ ] 所有 HTTP 出站过白名单拦截器
- [ ] Mivo 直连只走 `call_mivo_mcp` 的白名单工具，不开放任意 URL / 任意 method
...
| `D:\GIT\XD-AIGC-toolbox\AGENTS.md` | 涉及 toolbox 集成（服务器路径 / Conda / Gateway） |
